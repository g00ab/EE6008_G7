...
def classify_color(stats):
    ...
    if l > 85 and abs(a) < 10 and abs(b) < 10:
        return "white"
    elif 30 < a <= 40 and 30 < b <= 40:
        return "red"
    elif 0 < a <= 29 and 30 < b <= 70:
        return "orange"
    elif -28 < a < 0 and b > 40:
        return "yellow"
    elif a < -40 and -10 < b < 56:
        return "green"
    elif 0 < a < 15 and b < -25:
        return "blue"
    else:
        return "?"
...
while face_idx < max_faces:
    ...
    print(f"📸 Present side {face_idx + 1} of 6 and hold steady...")
    time.sleep(2)

    while len(face_map) < 9:
        print(f"Scanning {colour_order[face_idx]} face number {face_idx + 1}.....Detected {len(face_map)} positions.")
        color_votes = {}

        for _ in range(10):  # Vote over 5 frames
            img = sensor.snapshot()
            all_centers = []
            all_boxes = []
            ...
            for label, (cx, cy) in pos_map.items():
                ...
                for (x, y, w, h, class_id) in all_boxes:
                    if abs((x + w // 2) - cx) < 5 and abs((y + h // 2) - cy) < 5:
                        ...
                        if pred_label != "?" and pred_label != "":
                            color_votes[label].append(pred_label)
                        break
            time.sleep(0.1)

        # Final voting after 5 frames
        for label in position_names:
            if label in color_votes and color_votes[label]:
                most_common_color = max(set(color_votes[label]), key=color_votes[label].count)
                face_map[label] = most_common_color

    # Save the face
    center_color = face_map["center"]
    if center_color in faces_done:
        print(f"⚠️ Face with center color '{center_color}' already saved. Try a different one.")
    else:
        faces_done.append(center_color)
        cube_map[center_color] = face_map.copy()
        filename = f"side_{face_idx + 1}.jpg"
        img.save(filename)
        print(f"📸 Saved {filename} with center color '{center_color}'")
        print_face_3x3(face_map)
        face_idx += 1
        time.sleep(2)

# Final Step: Rebuild cube string
